...
app.post('/template/apply', async (req, res) => {
  ...
  try {
    ...
    try {
      addTaggedLog('INFO', '[APPLY]', '重启 Gateway 使配置生效...');
      
      // 先停止 Gateway（如果正在运行）
      if (gatewayState.process) {
        try {
          gatewayState.process.kill('SIGTERM');
          gatewayState.process = null;
          gatewayState.running = false;
          addTaggedLog('INFO', '[APPLY]', 'Gateway 进程已停止');
          await new Promise(resolve => setTimeout(resolve, 3000));
        } catch (killErr) {
          addTaggedLog('WARN', '[APPLY]', `停止 Gateway 失败: ${killErr.message}`);
        }
      } else {
        // 即使没有进程引用，也检查端口是否被占用
        if (checkGatewayRunning()) {
          try {
            ...
            addTaggedLog('INFO', '[APPLY]', '通过 CLI 停止 Gateway');
            await new Promise(resolve => setTimeout(resolve, 3000));
          } catch (stopErr) {
            addTaggedLog('WARN', '[APPLY]', `CLI 停止 Gateway 失败: ${stopErr.message?.substring(0, 100)}`);
          }
        }
      }
      ...
      // 重新启动 Gateway
      try {
        const openClawConfig = JSON.parse(readFileSync(OPENCLAW_CONFIG_FILE, 'utf-8'));
        const gatewayToken = openClawConfig?.gateway?.auth?.token || '';
        const env = {
          ...process.env,
          HOME: homedir,
        ...
        gatewayProcess.stdout.on('data', (data) => {
          const msg = data.toString().trim();
          if (msg) addLog('INFO', `[Gateway] ${msg}`);
        });
        gatewayProcess.stderr.on('data', (data) => {
          const msg = data.toString().trim();
          if (msg) addLog('WARN', `[Gateway] ${msg}`);
        });
        gatewayProcess.on('exit', (code) => {
          gatewayState.running = false;
          gatewayState.process = null;
          addLog('INFO', `Gateway 进程退出，代码: ${code}`);
        });
        ...
        await new Promise((resolve) => {
          const checkInterval = setInterval(() => {
            ...
            if (checkGatewayRunning()) {
              clearInterval(checkInterval);
              gatewayState.running = true;
              addTaggedLog('INFO', '[APPLY]', 'Gateway 重启成功，服务已就绪');
              resolve();
            }
            if (checkCount >= maxChecks) {
              clearInterval(checkInterval);
              addTaggedLog('WARN', '[APPLY]', `Gateway 重启后等待超时（${maxChecks * 0.5}秒），端口未就绪`);
              resolve();
            }
          }, 500);
        });
      } catch (startErr) {
        addTaggedLog('WARN', '[APPLY]', `Gateway 重启失败: ${startErr.message}`);
      }
    } catch (restartErr) {
